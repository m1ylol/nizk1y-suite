...
class CurrencyConverter {
    ...
    swapValues() {
        const rublesValue = this.rublesInput.value;
        const centimetersValue = this.centimetersInput.value;
        
        this.rublesInput.value = centimetersValue;
        this.centimetersInput.value = rublesValue;
        
        this.converterIcon.style.transform = 'rotate(180deg) scale(1.2)';
        setTimeout(() => {
            this.converterIcon.style.transform = 'rotate(0deg) scale(1)';
        }, 300);
    }
...
class AudioPlayer {
    ...
    updateMuteButton() {
        this.muteBtn.innerHTML = this.audio.volume === 0 ? '<span class="emoji">🔇</span>' : '<span class="emoji">🔊</span>';
